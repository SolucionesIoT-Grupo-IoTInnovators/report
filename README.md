...
| 44      | MS01           | Detectar estado de ocupación del spot      | Como sistema IoT embebido, quiero detectar si un spot está ocupado o libre para reportarlo al sistema.                            | 3                                |
| 45      | MS02           | Bloquear spot reservado                    | Como sistema, quiero bloquear un spot cuando está reservado para evitar que otro usuario lo ocupe.                                | 5                                |
| 46      | MS03           | Encender LED según estado del spot         | Como conductor, quiero ver una luz LED que indique si el spot está libre, reservado u ocupado.                                     | 3                                |
| 47      | MS04           | Mostrar nombre del spot en el display      | Como usuario, quiero ver el nombre del spot en el display del dispositivo para facilitar su identificación.                        | 2                                |
| 48      | MS05           | Mostrar MAC del dispositivo en el display  | Como técnico, quiero ver la MAC del dispositivo para enlazarlo con el spot correspondiente.                                        | 2                                |
| 49      | MS06           | Enviar datos al backend desde el dispositivo | Como dispositivo embebido, quiero enviar los datos recogidos al backend para ser procesados.                                       | 3                                |
| 50      | MS07           | Guardar datos sin conexión WiFi            | Como sistema IoT, quiero guardar datos localmente si no hay WiFi para no perder lecturas.                                          | 3                                |
| 51      | MS08           | Sincronizar datos cuando vuelve la red WiFi | Como sistema IoT, quiero sincronizar los datos almacenados localmente cuando se recupere la red WiFi.                             | 3                                |
| 52      | MS09           | Backend recibe métricas de sensores        | Como backend, quiero recibir las métricas de los sensores para almacenarlas y analizarlas.                                         | 2                                |
| 53      | MS10           | Backend procesa datos de ocupación         | Como backend, quiero procesar datos de ocupación para actualizar el estado en el panel.                                            | 3                                |
| 54      | MS11           | Backend genera alertas por sensores        | Como sistema, quiero que el backend genere alertas si se detectan comportamientos anómalos.                                        | 5                                |
| 55      | MS12           | Backend almacena métricas históricas       | Como administrador, quiero acceder al histórico de métricas para análisis posteriores y reportes.                                  | 2                                |
...
| TS05 | Implementar sistema de pagos en línea | 12 | Integración pasarela de pago | Conectar con pasarela de pagos en app | 8h |    | Done |
